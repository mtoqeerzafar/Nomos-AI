"""
Security Test Suite: Input/Output Guardrails & PII Sanitization (`tests/run_guardrail_tests.py`)
Purpose: Evaluates security middleware guardrails (`security/guardrails/`).
Functionality: Tests PII redaction (phone numbers, email addresses, names) and prompt injection vulnerability detection
(jailbreak attempts, system instruction overrides). Confirms malicious prompts are blocked before entering the agent graph.
Usage: Run via `python tests/run_guardrail_tests.py` or `pytest`.
"""
