"""
Unit Test Suite: Planner Agent Intent & Strategy Routing (`tests/test_planner.py`)
Purpose: Validates PlannerAgent (v1.2) intent classification, retrieval necessity detection, and language lock rules.
Functionality: Executes assertions against conversational greetings (no retrieval), law lookups (retrieval required),
and multi-turn chat history evaluation. Verifies that the Planner outputs valid PlannerDecision schemas.
Usage: Run via `pytest tests/test_planner.py` or as part of automated CI regression suites.
"""
