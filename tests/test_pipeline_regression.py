"""
Regression Test Suite: Pipeline Safety & Multi-Turn Stability (`tests/test_pipeline_regression.py`)
Purpose: Evaluates production `AgentWorkflow` against multi-turn regression query sequences.
Functionality: Connects live hybrid retriever with full agentic graph to verify context retention across follow-up queries
(e.g., bullet point summaries, mathematical formulas, and topic transitions). Confirms zero turn failures or state corruption.
Usage: Run via `python tests/test_pipeline_regression.py` or `pytest`.
"""
