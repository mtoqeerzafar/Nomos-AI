"""
Regression Evaluation Suite: Memory & Database Integration (`tests/evaluate_regression.py`)
Purpose: Evaluates production `AgentWorkflow` interactions with PostgreSQL relational tables (`db/models.py`) and conversation memory.
Functionality: Creates ephemeral test chat threads, persists multi-turn message exchanges, verifies memory compression boundaries,
and audits turn status logging (`success`, `refusal`, `fallback`).
Usage: Run via `python tests/evaluate_regression.py`.
"""
