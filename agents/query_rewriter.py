    ...
    def _generate_rewrite(self, original_question: str, chat_history: str, issues: list = None, needs_chat_history: bool = False) -> str:
        ...
        **Expected JSON Output Schema:**
        ...
        try:
            ...
            # Preserve explicit legal entities from original question
            from utils.query_entity_extractor import extract_query_entities
            orig_entities = extract_query_entities(original_question)
            if orig_entities.has_citation:
                for art in orig_entities.articles:
                    art_str = f"المادة {art}"
                    if art_str not in result["standalone_query"] and f"Article {art}" not in result["standalone_query"]:
                        result["standalone_query"] = f"{result['standalone_query']} ({art_str})"
                for lnum in orig_entities.law_numbers:
                    law_str = f"القانون {lnum}"
                    if lnum not in result["standalone_query"]:
                        result["standalone_query"] = f"{result['standalone_query']} ({law_str})"
