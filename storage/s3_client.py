...
class S3Client:
        ...
        try:
            ...
            if error_code == '404':
                ...
                try:
                    ...
                    logger.warning(f"Could not create bucket {self.bucket_name}: {create_err}")
            else:
                logger.warning(f"Error checking bucket {self.bucket_name}: {e}")
        except Exception as e:
            logger.warning(f"Could not connect to S3 endpoint to check bucket {self.bucket_name}: {e}")
    ...
    def delete_file(self, object_name: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Successfully deleted {object_name} from storage")
            return True
        except ClientError as e:
